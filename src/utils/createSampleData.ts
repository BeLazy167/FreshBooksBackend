...
const generateRandomVegetables = (count: number): VegetableItem[] => {
    return Array.from({ length: count }, () => ({
        ...
        hasFixedPrice: Math.random() < 0.5,
        fixedPrice: parseFloat((Math.random() * 5 + 0.5).toFixed(2)),
    }));
};
...
export const createSampleProviders = (count: number): Provider[] => {
    return Array.from({ length: count }, (_, index) => ({
        ...
        } Sample St, City, State, 12345`,
    }));
};

export const createSampleBills = (
    count: number,
    providers: Provider[]
): Bill[] => {
    return Array.from({ length: count }, () => {
        ...
        const items = generateRandomVegetables(
            Math.floor(Math.random() * vegetables.length) + 1
        );
        const total = items
            .reduce((sum, item) => sum + item.quantity * item.fixedPrice, 0)
            .toFixed(2);
